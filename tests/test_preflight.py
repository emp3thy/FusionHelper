...
def test_expect_stub_true_default_checks_r8(tmp_path):
    r = preflight.run_preflight(write(tmp_path, GOOD))
    assert "checked: R1 R2 R4 R5 R6 R7 R8 R10 R11 ·" in r.report
    assert "not checked: R3 R9" in r.report
...
def test_expect_stub_false_moves_r8_to_not_checked(tmp_path):
    ...
    assert "checked: R1 R2 R4 R5 R6 R7 R10 R11 ·" in r.report
    assert "not checked: R3 R8 R9" in r.report
