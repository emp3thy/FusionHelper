...
## Guardrails (from better-memory, retrieved before execution)

High-confidence reflections that bind this plan. Numbers are confidence / evidence_count / useful_count at retrieval time.

- **[[docs-in-sync]]** (0.95 / 7 / 20) — docs change in the SAME task as the code they describe, never a follow-up. Applied: Task 6 updates SKILL.md and api-notes §15 in the same commit cycle as the calibration script; Task 5's R8 semantics change carries its own test file documenting the new modes. Any task that adds/renames a CLI flag must have its doc text verified against the argparse setup, not paraphrased — Task 4's usage line must match `bundle.main()` exactly.
- **[[step-gating]]** (0.90 / 0 / 0, measured on this project's LED-wall drivers) — multi-step flows gate every step on its own explicit success signal; a final green certifies only the final artifact, not the history that produced it. Applied: Task 4 step 6 preflights the bundled artifact as its own gated step; the SKILL.md workflow line in Task 6 mandates re-bundle → grep artifact for the edited symbol → launch as separate gated commands. Bundler CLI exits non-zero on every failure path (no silent no-op).
- **[[red-step-genuinely-red]]** (0.75 / 2 / 2) — before claiming a TDD step fails, confirm nothing deeper already satisfies the assertion. Applied: each task's "Expected: FAIL with X" names the exact first error (ModuleNotFoundError / AttributeError / ImportError); implementers must confirm that exact failure, not just any red. Task 5 is the risk case: `check_text` already fails author scripts (stub missing) — the new tests assert the AUTHOR-mode message text ("bundler owns the stub"), which no existing code emits, so red is genuine.
- **[[planning-memories-first]]** (0.90 / 1 / 16) — this section exists because of it: planning memories retrieved and surfaced before execution. Executing agents: if you discover a conflict between a guardrail and a task, stop and surface it, do not silently pick one.

Dismissed after review (considered, not applicable): freeze-localization logging (0.55 — no long-running sync paths in this plan); mkstemp/fdopen fd leak (0.60 — bundler uses plain `Path.write_text`, no mkstemp); TypeScript Partial<T> excess-property (TS-only); ralph-queue dispatch boundary (work not queued to ralph).

- **[[per-task-confidence]]** (standard: `standards/ralph-runtime.md`, "Apply confidence scoring to implementation plans" — cross-project, NOT ralph-only) — every task carries a confidence percentage; tasks ≤90% get mitigations embedded in the task body until lifted above 90%, or are surfaced explicitly as unliftable. Verify-before-commit was run at plan-write time (see per-task notes).

## Per-Task Confidence

| Task | Confidence | Basis and lift actions taken at plan-write time |
|------|-----------|--------------------------------------------------|
| 1 BuildCtx + sketch helpers | **93%** | Helper bodies are live-proven build16 v16 code, transcribed not re-derived. Fake-adsk scaffold verified attribute-by-attribute against every read in `BuildCtx.__init__` (features/constructionPlanes/axes/userParameters). Residual: fake-import ordering fragility, contained by the sys.modules-before-import pattern proven in test_verify_offline.py. |
| 2 Cut/join helpers | **92%** | FakeExtrudes walked through each test by hand: direction-flip consumes effect 1+2, cache path consumes exactly one effect on second call. Volume thresholds mirror measured semantics (delta > 0.02 cm³). |
| 3 Pattern helpers | **91%** | FakePatterns walkthrough: raise → retry ladder consumes +d Adjust/Identical then validates on −d; healthState string-enum membership works with the _Enum fake; `inp.dd[1]` matches the `("VI", expr)` cbs fake. Threshold-exclusivity ValueError tested both ways. |
| 4 Bundler | **91%** | Lifted from 85%: verified `verify.append_to`/`STUB_SENTINEL` exist and are the exact API R8 uses today; verified the artifact carries NO `fusionhelper` import (kit imports only adsk, author's import line is replaced) so the pyright gate stays clean; verified kit loops trigger only R11 WARNs which do not fail preflight. Step 6 gates the artifact through real preflight. Added `test_artifact_compiles` (below) so a syntactically broken splice cannot survive the offline suite. |
| 5 R8 author mode + dispatch | **93%** | Lifted: `fusionhelper/preflight/__init__.py:54-55` READ at plan-write time — the dispatch edit below is exact, no adaptation needed. Red-step genuinely red: new tests assert author-mode message text no existing code emits. Note: running preflight directly on an author script may also emit pyright import errors — that is the DESIRED outcome (an author must never reach Fusion); the R8 author-mode finding supplies the actionable message. |
| 6 Calibration + docs | **85% — surfaced, not lifted** | The offline-verifiable parts are checked (pos-expression contract: `u_pos=("cal_w / 2", "cal_w / 2")` puts the corner at 0; pattern threshold 0.06 cm³ < 2 × 0.0377 cm³ actual). The residual is the first live-Fusion run of the kit — which is this task's PURPOSE: it exists to absorb exactly that risk. Gate: the kit is not advertised as proven in SKILL.md until this step runs green once; failure loops back into Tasks 1-4 as fixes. |
...
(Verified against the file at plan-write time: lines 54-55 are exactly `if expect_stub:` / `findings.extend(r8_stub_intact.check_text(source))` — this edit replaces that call with the dispatch verbatim; `findings` and `source` are the in-scope names. Legacy scripts with no buildkit import are untouched.)
