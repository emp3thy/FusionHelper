...
def check_text(source: str) -> list[Finding]:
    ...
    if verify.STUB_SENTINEL not in source:
        ...
        # Diagnose from the LAST sentinel onward: intact stub text followed by
        # more content = appended code; anything else = modified stub.
        tail = _norm(source[source.rfind(verify.STUB_SENTINEL):])
        if tail.startswith(_norm(verify.STUB_TEXT)):
...
8. **Repair loop.** Budgets: preflight fixes 3 (offline, separate); runtime with
   ...
   **5 `fusion_mcp_execute` calls per request**. Abort early when: the identical
   ...
   fails to strictly decrease twice; a repair introduces a new error code
   without clearing the old one (counts as no progress). Third attempt is
   always a from-scratch regeneration, never a patch. `model.not_restored`,
   `model.inert`, `edit.introduces_clash` or a timeline error state → undo
