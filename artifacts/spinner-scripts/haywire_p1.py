"""HAYWIRE GEARWORKS 76 - part 1: the stator.

A spinner that looks like a failed print - a rim shattered into five
chunks with daylight through the gaps, held on by what reads as drooping
extruder stringing (one strand deliberately snapped) - and which spins
glass-smooth while five planet gears whirl visibly in the gaps. At speed
the five chunks fuse by persistence of vision into a solid ring and the
web disappears: broken at rest, whole at speed.

Part 1 builds the stator: base disc, 55-tooth internal ring gear wall,
45-degree centre journal post, stem and cap head. Gear train is
module 1.0, ring 55T / five planets 14T -> 3.93 planet turns per carrier
revolution, 11 teeth per station so every station meshes in phase.

Printability rules held throughout: every overhang 45 degrees or
shallower, no unanchored floating islands, 0.25 mm normal journal
clearance (0.354 mm radial on the 45-degree flanks).
"""
import math
...
def run(_context: str):
    ...
    up.add("hw_base_t", cbs("2 mm"), "mm", "stator base thickness")
    up.add("hw_wall_h", cbs("9 mm"), "mm", "ring gear wall height")
    up.add("hw_fuse_ch", cbs("0.3 mm"), "mm", "anti-fuse chamfer")
    ...
    skb.name = "base_disc"
    ctx.bound_circle(skb, (0, 0, 0), BASE_R, "63.2 mm",
                     x_pos="0 mm", v_pos="0 mm")
    dinp = ctx.extrudes.createInput(skb.profiles.item(0),
                                    ctx.ops.NewBodyFeatureOperation)
    ext = adsk.fusion.DistanceExtentDefinition.create(cbs("hw_base_t"))
    ...
    ctx.bound_circle(skt, (0, 0, 0), WALL_ROUT, "63.2 mm",
                     x_pos="0 mm", v_pos="0 mm")
    _fix_sketch(skt)
    ...
    tinp.startExtent = adsk.fusion.OffsetStartDefinition.create(
        cbs("hw_base_t"))
    ext2 = adsk.fusion.DistanceExtentDefinition.create(cbs("hw_wall_h"))
