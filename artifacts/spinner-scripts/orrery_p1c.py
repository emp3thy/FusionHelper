"""ORRERY 90 - part 1c: journal post + cleanup of the orphaned retry.
...
def run(_context: str):
    ...
    if stator is None or root.bRepBodies.count != 1:
        raise RuntimeError("expected exactly or_stator")
    if not (10.5 < stator.volume < 12.0):
        raise RuntimeError("expected base+ring (~11.24), got %.3f"
                           % stator.volume)

    # a killed client's request still executes: the orphaned retry left a
    # duplicate sketch. Drop it and any stale post sketch before rebuilding.
    for sk in list(root.sketches):
        adsk.doEvents()
        if sk.name.startswith("ring_teeth (") or sk.name == "post_profile":
            print("FH removing stale sketch:", sk.name)
            sk.deleteMe()
