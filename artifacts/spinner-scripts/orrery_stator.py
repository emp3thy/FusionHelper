"""ORRERY 90 - the stator, idempotent.

Built in one script that can be re-entered safely: each stage checks the
current volume and skips work already committed. That matters because a
client timeout here does NOT abort the script - Fusion finishes it, and
the killed client's request is re-queued and RUNS AGAIN. A non-idempotent
script duplicates geometry when that happens (measured: it left a stray
'ring_teeth (1)' sketch).
...
def run(_context: str):
    ...
    for sk in list(root.sketches):
        adsk.doEvents()
        if "(" in sk.name:
            print("FH dropping duplicate sketch:", sk.name)
            sk.deleteMe()
    done = root.bRepBodies.itemByName("or_stator")
    vol = done.volume if done else 0.0
    print("FH stator resume: %.3f cm3" % vol)
