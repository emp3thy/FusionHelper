...
outside the finger dimple). Depth is a live parameter sn2_engrave_d.
"""
import math
...
def run(_context: str):
    ...
    for i in range(app.documents.count):
        ...
        if des and des.rootComponent.bRepBodies.itemByName("sn2_rotor"):
            target = doc
            break
    if target is None:
        raise RuntimeError("Supernova rev B doc not found")
    target.activate()
    adsk.doEvents()
    ...
    pt = ctx.pt
    cbs = ctx.cbs
    rotor = root.bRepBodies.itemByName("sn2_rotor")
    stator = root.bRepBodies.itemByName("sn2_stator")

    ctx.up.add("sn2_engrave_d", cbs("0.5 mm"), "mm", "engrave depth")
    ...
    f1 = ctx.blind_cut(ring_prof, "sn2_engrave_d", [rotor],
    ...
    f2 = ctx.blind_cut(ctx.all_profiles(skt), "sn2_engrave_d", [stator],
