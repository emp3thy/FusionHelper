"""HAYWIRE GEARWORKS 76 - part 3: the five planet gears.

14-tooth planets on the rotor's mini diamond stubs, meshing with the
stator's 55-tooth internal ring. One flick of the shattered rim and each
planet turns 55/14 = 3.93 times per carrier revolution, whirling in the
daylight gaps. 55/5 = 11 teeth per station, so the 72-degree body
pattern lands every station in mesh phase.

Each planet carries an offset puck so the eye can actually see the
rotation instead of a blur of teeth.
"""
import math
...
FH_ATTEMPT = 3
FH_OPTS = {"only_params": ["hw_base_t", "hw_wall_h", "hw_fuse_ch"]}
...
def run(_context: str):
    ...
    rotor = root.bRepBodies.itemByName("hw_rotor")
    if stator is None or rotor is None or root.bRepBodies.count != 2:
        raise RuntimeError("expected hw_stator + hw_rotor from parts 1-2")
    ...
    planet.name = "hw_planet_1"
    if not (0.4 < planet.volume < 1.1):
    ...
    for i in range(pfb.bodies.count):
        pfb.bodies.item(i).name = "hw_planet_%d" % (i + 2)
